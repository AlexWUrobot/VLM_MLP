...
def _match_filename_prefix(name: str) -> tuple[str, str] | None:
    """Return (source_prefix, target_label) for a filename stem, or None.

    source_prefix is the actual string in the filename (e.g. 'idle_back').
    target_label is the merged 6-class label (e.g. 'idle').
    """
    for prefix in _ALL_FILENAME_LABELS:
        if name == prefix or name.startswith(prefix + "_"):
            target = "idle" if prefix in _MERGE_TO_IDLE else prefix
...
def infer_label_from_filename(name: str) -> str | None:
    ...
    match = _match_filename_prefix(name)
    return match[1] if match else None


def parse_clip_id_and_time_ms_from_path(path_str: str) -> tuple[str, int, int] | None:
    """Parse filenames like '<source_prefix>_<clipId>_<time>ms.jpg'.

    Returns (source_prefix, clip_id, time_ms) or None if the pattern doesn't match.
    source_prefix is the actual filename prefix (e.g. 'idle_back', not 'idle').
    """
    stem = Path(path_str).stem
    match = _match_filename_prefix(stem)
    if match is None:
        return None
    source_prefix, _target = match

    rest = stem[len(source_prefix) :]
    ...
    return source_prefix, clip_id, t_ms
...
def save_cache(
    ...
    ys: np.ndarray,
    paths: list[str],
    phone_probe_scores: np.ndarray | None = None,
    phone_detected: np.ndarray | None = None,
) -> None:
    ...
    if phone_probe_scores is not None:
        payload["phone_probe_scores"] = phone_probe_scores.astype(np.float32, copy=False)
    if phone_detected is not None:
        payload["phone_detected"] = phone_detected.astype(np.float32, copy=False)
    np.savez_compressed(cache_path, **payload)


def load_cache(cache_path: Path) -> tuple[np.ndarray, np.ndarray, list[str], np.ndarray | None, np.ndarray | None]:
    data = np.load(cache_path, allow_pickle=True)
    feats = data["feats"]
    ys = data["ys"]
    paths = data["paths"].tolist()
    phone_probe_scores = data["phone_probe_scores"] if "phone_probe_scores" in data.files else None
    phone_detected = data["phone_detected"] if "phone_detected" in data.files else None
    return feats, ys, paths, phone_probe_scores, phone_detected
...
def compute_phone_detected_features(
    ...
    Returns (N, 1) float32 array: 1.0 if phone detected, else 0.0.
    """
    import cv2
    from ultralytics import YOLO

    yolo = YOLO(yolo_model_path)
    detected = np.zeros((len(paths), 1), dtype=np.float32)
    for i, p in enumerate(tqdm(paths, desc="YOLO phone detection")):
        img = cv2.imread(str(p))
        if img is None:
            continue
        results = yolo.predict(img, conf=conf, classes=[COCO_PHONE_CLASS], verbose=False)
        r = results[0]
...
def build_temporal_windows(
    ...
    for i, p in enumerate(paths):
        ...
        if parsed is None:
            continue
        source_prefix, clip_id, t_ms = parsed
        # Use source_prefix for key so idle_back_0 and idle_front_0 stay separate
        key = f"{source_prefix}__{clip_id}"
...
def main() -> None:
    ...
    parser.add_argument("--num-workers", type=int, default=4, help="DataLoader workers for CLIP feature extraction")
    ...
    parser.add_argument("--yolo", default="yolo11n.pt", help="YOLO model for phone detection on training images")
    ...
    if (not args.no_cache) and cache_path.exists():
        feats, ys, paths, phone_probe_scores, phone_detected = load_cache(cache_path)
        ...
        feats, ys, paths = extract_clip_features(
            samples,
        ...
        phone_probe_scores = None
        phone_detected = None

    clip_dim = int(feats.shape[1])

    # Compute CLIP zero-shot phone probe scores
    if phone_probe_scores is None:
        phone_probe_scores = compute_phone_probe_scores(feats, device)
    print(f"Phone probe: {phone_probe_scores.shape[1]} prompts, scores shape={phone_probe_scores.shape}")

    # Compute YOLO phone detection feature (binary 0/1 per image)
    if phone_detected is None:
        phone_detected = compute_phone_detected_features(paths, args.yolo)

    # Save cache with both phone features
    if not args.no_cache:
        save_cache(cache_path, feats, ys, paths, phone_probe_scores, phone_detected)
        print(f"Saved per-frame feature cache: {cache_path}")

    # Concatenate: phone_probe(5) + phone_detected(1) = 6 dims
    phone_all = np.concatenate([phone_probe_scores, phone_detected], axis=1).astype(np.float32)
    phone_probe_dim = int(phone_all.shape[1])
    n_det = int(phone_detected.sum())
    print(f"Phone features: probe={phone_probe_scores.shape[1]} + detected={phone_detected.shape[1]} => {phone_probe_dim} dims | {n_det}/{len(paths)} images with phone detected")
    ...
    for i, p in enumerate(paths):
        ...
        combined, _hand_mask = combine_clip_pose_hands_handmask(feats[i], pose, hands, phone_all[i])
