...
# Old labels that should map to "idle"
_MERGE_TO_IDLE = {"idle_back", "idle_front", "none"}

# All filename prefixes we recognise (old 8 + new merged)
...
class CLIPTemporalGRU(nn.Module):
    ...
    Input:  flat concat vector of shape (B, num_frames * clip_dim)
    The forward method reshapes it to (B, num_frames, clip_dim), runs the GRU,
    and classifies from the last hidden state.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, num_frames * clip_dim)  -- flat concat from build_temporal_windows
        B = x.shape[0]
        seq = x.view(B, self.num_frames, self.clip_dim)  # (B, T, D)

        out, hidden = self.gru(seq)  # hidden: (layers*dirs, B, rnn_hidden)

        if self.bidirectional:
            last_hidden = torch.cat([hidden[-2], hidden[-1]], dim=1)
        else:
            last_hidden = hidden[-1]  # (B, rnn_hidden)
...
def build_temporal_windows(
    ...
    When use_diffs=True and feat_agg='concat', appends (N-1) frame-difference
    vectors so the MLP can see motion between frames.  This is critical for
    distinguishing static gestures (stop) from dynamic ones (come).

    Output dim per window:
      feat_agg='concat', use_diffs=False:  N * D
      feat_agg='concat', use_diffs=True:   N * D + (N-1) * D = (2N-1) * D
      feat_agg='mean':                     D  (diffs ignored)
    """
        ...
        for j in range(0, len(frame_idxs) - num_frames + 1, stride):
            ...
            if feat_agg == "mean":
                ...
                if use_diffs and num_frames > 1:
                    diffs = f[1:] - f[:-1]  # (num_frames-1, base_dim)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Append inter-frame difference features (critical for come vs stop). Default: on.",
    )
    parser.add_argument(
        "--no-diffs",
        action="store_true",
        help="Disable inter-frame difference features.",
    )
    ...
    # GRU processes raw frame sequences; diffs only make sense for MLP
    if args.temporal_model == "gru":
        use_diffs = False
        if args.feat_agg != "concat":
            raise SystemExit("GRU requires --feat-agg=concat to preserve frame order.")
    else:
        use_diffs = args.use_diffs and (not args.no_diffs)
    ...
    print(
        f"Temporal windows: X={X.shape} (base_dim={base_dim}, diffs={use_diffs}) | "
