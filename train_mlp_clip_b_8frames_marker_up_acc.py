...
def load_cache(cache_path: Path) -> tuple[np.ndarray, np.ndarray, list[str], np.ndarray | None]:
    data = np.load(cache_path, allow_pickle=True)
    feats = data["feats"]
    ys = data["ys"]
    paths = data["paths"].tolist()
    phone_probe_scores = data["phone_probe_scores"] if "phone_probe_scores" in data.files else None
    return feats, ys, paths, phone_probe_scores
...
def main() -> None:
    ...
    if (not args.no_cache) and cache_path.exists():
        feats, ys, paths, phone_probe_scores = load_cache(cache_path)
    ...
    if phone_probe_scores is None:
        ...
        if not args.no_cache:
            save_cache(cache_path, feats, ys, paths, phone_probe_scores)
