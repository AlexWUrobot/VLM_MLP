...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--yolo", default=_default_yolo_model(), help="Ultralytics YOLO model for person detection")
...
def _default_yolo_model() -> str:
    engine_path = Path(__file__).with_name("yolo11n.engine")
...
def main() -> None:
    ...
    try:
        while True:
            ...
            dt_grab_ms = (time.perf_counter() - t_grab) * 1000.0
            if frame is None:
                break
            ...
            dt_yolo_ms = (time.perf_counter() - t_yolo) * 1000.0
            ...
            if do_predict:
                ...
                if boxes:
                    ...
                    if crops:
                        ...
                        dt_clip_ms = (time.perf_counter() - t_clip) * 1000.0
                        ...
                        for j, box_i in enumerate(crop_to_box_i):
                            ...
                            dt_body_ms += (time.perf_counter() - t_body) * 1000.0

                            t_hand = time.perf_counter()
                            hand_result = hand_landmarker.detect(mp_image)
                            dt_hand_ms += (time.perf_counter() - t_hand) * 1000.0
            ...
            hud_lines = [
                ...
                f"display fps: {ema_display_fps:.1f}",
                f"camera fps: {cam_fps:.1f}",
                f"avg fps: {avg_fps:.1f}",
                f"grab: {dt_grab_ms:.0f} ms",
                f"yolo: {dt_yolo_ms:.0f} ms",
                f"clip: {dt_clip_ms:.0f} ms",
                f"body: {dt_body_ms:.0f} ms",
                f"hand: {dt_hand_ms:.0f} ms",
                f"show: {dt_show_ms:.0f} ms",
            ]
            ...
            dt_show_ms = (time.perf_counter() - t_show) * 1000.0
            ...
            prev_loop_end = now
            instant_fps = 1.0 / loop_dt if loop_dt > 0 else 0.0
            if ema_display_fps > 0.0:
                ema_display_fps = ema_alpha_fps * ema_display_fps + (1.0 - ema_alpha_fps) * instant_fps
            else:
                ema_display_fps = instant_fps

            if frame_i % 30 == 0:
                print(
                    f"[perf] cam={cam_fps:.1f}fps display={ema_display_fps:.1f}fps "
                    f"grab={dt_grab_ms:.0f}ms yolo={dt_yolo_ms:.0f}ms clip={dt_clip_ms:.0f}ms "
                    f"body={dt_body_ms:.0f}ms hand={dt_hand_ms:.0f}ms show={dt_show_ms:.0f}ms"
                )
    finally:
        reader.stop()
        try:
